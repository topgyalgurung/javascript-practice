document.addEventListener("DOMContentLoaded", function () {
  ...
  let noteIdCounter = localStorage.getItem("noteIdCounter") || 0;
  ...
  function addNewNote() {
    ...
    // read note first, add to it then save it. also update the idcounter
    const notes = readNotes(); // readNotes and get the return value into notes variable
    notes.push({ id, content }); // add the content
    saveNotes(notes); // save the notes calling saveNotes function
  ...
  noteContainer.addEventListener(
    ...
    function (event) {
      if (event.target.classList.contains("note")) {
        ...
        const id = Number(event.getAttribute("data-note-id"));
        const notes = readNotes();
        for (let i = 0; i < notes.length; i++) {
          if (notes[i].id === id) {
